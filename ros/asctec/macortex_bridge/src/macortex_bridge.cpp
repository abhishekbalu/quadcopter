...
void MyDataHandler(sFrameOfData* FrameOfData)
{
  ...
  quad_msgs::MultiPose quad_poses_msg;
  quad_poses_msg.header.stamp = ros::Time::now();
  //mcs_pose_msg.header.frame_id - not important
  for(int k=0;k<nquads;k++)
  {
    //only send quadrotor data to the topic message if a new measurement was received for it
    if(quad_pose_list[k].pose_updated == 1){
      quad_msgs::SinglePose quad_pose;
...
int main(int argc, char* argv[])
{
  ...
  quad_poses = nh.advertise<quad_msgs::MultiPose>("macortex_bridge/quad_poses",1);
