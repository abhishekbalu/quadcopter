...
// normal distribution for adding noise to velocity measurements
/*double vel_sigma = 1;
normal_distribution<double> sensor_noise(0.0, vel_sigma);
...
void compute_force(Eigen::Vector3d & force, double &yaw_torque)
{
	...
	if(!strcmp(this_quad.c_str(), "quad97")){ // follower
		angle -= 0.0*PI/180;
	...
	//control based on velocity error (followers in teleoperation case)
		...
		force << 0,0,0; //force = -0.5*R.inverse()*coefs.Kv*controller.ev;
	//control based on velocity error, but absolute z height also controlled (leader in teleoperation case, otherwise leader falls down)
		...
		force = -0.5*R.inverse()*coefs.Kv*controller.ev;
...
void internal_ekf_input_callback(const internal_ekf::EstimateSingle::ConstPtr& estimates_msg)
{
	...
	// // NOISE ADDED TO THE HORIZONTAL VELOCITY MEASUREMENT
	// parameters of the distribution are defined upon initialization (near top) 
	// state.speed(0) += sensor_noise(generator);
	// state.speed(1) += sensor_noise(generator);
}
...
void algorithm_input_callback(const main_control::state_command::ConstPtr& new_target)
{
	...
	// comment this out if lag is being simulated below
	target.speed(0) = new_target->velocity.x;
	target.speed(1) = new_target->velocity.y;
	target.speed(2) = new_target->velocity.z;

	// the velocity has to be added, otherwise it overwrites
	// the velocity command from vgoto
