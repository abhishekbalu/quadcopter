...
int detect_markers()
{
    ...
    for(int i1=0;i1<no;i1++){
        ...
        for(int i2=i1+1;i2<no;i2++){
            ...
            for(int i3=i2+1;i3<no;i3++){
                                       ...
                                       for(int j4=0;j4<4;j4++){
                                           ...
                                           for(unsigned l=0;l<4;l++){
                                               ...
                                               if(lcost<cost){
                                                    ...
                                                    cout<<Tl<<endl;
                                                    cout<<lcost<<endl;*/
						   //these conditions are set for the camera reference frame (z front, y down, x right)
			    ...
			    //validating cost
			    if(cost <= (0.5*(fx+fy)/Tsol(2,3))*0.05){
    ...
    //cout << "flag1" << endl; //*FLAG
				...
				//distort and create square on the quad
				prx=cx+trans1(0)*(fx/trans1(2)); px=prx-cx;
				pry=cy+trans1(1)*(fy/trans1(2)); py=pry-cy;
				for(int k=0;k<5;k++){
				    r=px*px + py*py;
    ...
    //cout << "flag2" << endl; //*FLAG
				...
				//distort and create square on the quad red led
    ...
    //cout << "flag3" << endl; //*FLAG
				...
				//cout<<"cost: "<<cost<<" limit: "<<(0.5*(fx+fy)/Tsol(2,3))*0.05<<endl;
				Tsol=camR*Tsol;
...
*/
Eigen::MatrixXd* sensor_init(int width,int height,
                             double fx_l,double fy_l,double cx_l,double cy_l,double kx1_l,double kx2_l,double ky1_l,double ky2_l, double dt_min_l)
{
