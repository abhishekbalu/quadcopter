...
double objQuad[]={0.24,0.0,0.0,0.0,0.24,0.0,0.06,0.05,0.06,0.0,0.0,0.21,0.08,0.08,0.0};
...
int track_markers(unsigned char* buf,unsigned int step)
{
    ...
    //detect blobs
    detect_blobs(buf,step,40,100,170,230,40,100,0,nx,0,ny-40);
    //cout<<"no="<<no<<endl;
    ...
    for(vector<marker>::iterator it = marker_list.begin() ; it != marker_list.end(); ++it){
        ...
        if(nlobjects>0){
            ...
            it->imx0=imx0[min_ind]; it->imx0_LED=imx0_LED[k];
            it->imxf=imxf[min_ind]; it->imxf_LED=imxf_LED[k];
            it->imy0=imy0[min_ind]; it->imy0_LED=imy0_LED[k];
            it->imyf=imyf[min_ind]; it->imyf_LED=imyf_LED[k];
    ...
    for(int k=0;k<nerase;k++){
        marker_list.erase(marker_list.begin()+erase[k]);
        nmarkers--;
    }
        ...
        if(it->max>4){ //THRESHOLD
            ...
            for(int l=0;l<10;l++)
                if(it->frequencies[l]>4){
                    freq+=it->frequencies[l]*l;
                    total+=it->frequencies[l];
                }
            freq/=total;
            //cout<<freq<<endl;
                ...
                for(int i4=i3+1;i4<no;i4++){
                    ...
                    //if((abs(sz1/avg - avg)<thsz)&&(abs(sz2/avg - avg)<thsz)&&(abs(sz3/avg - avg)<thsz)&&(abs(sz3/avg - avg)<thsz))
                    {
                        ...
                        if((d1<avg*thd)&&(d2<avg*thd)&&(d3<avg*thd))
                        {
                            ...
                            objcnt++;
                            trans1=Tsol.col(3);
                            imx0[objcnt-1]=cx+trans1(0)*(fx/trans1(2))-szx*(fx/trans1(2));
                            imxf[objcnt-1]=cx+trans1(0)*(fx/trans1(2))+szx*(fx/trans1(2));
                            imy0[objcnt-1]=cy+trans1(1)*(fy/trans1(2))-szy*(fy/trans1(2));
                            imyf[objcnt-1]=cy+trans1(1)*(fy/trans1(2))+szy*(fy/trans1(2));
                            ...
                            imx0_LED[objcnt-1]=cx+trans1(0)*(fx/trans1(2))-szLED*(fx/trans1(2));
                            imxf_LED[objcnt-1]=cx+trans1(0)*(fx/trans1(2))+szLED*(fx/trans1(2));
                            imy0_LED[objcnt-1]=cy+trans1(1)*(fy/trans1(2))-szLED*(fy/trans1(2));
                            imyf_LED[objcnt-1]=cy+trans1(1)*(fy/trans1(2))+szLED*(fy/trans1(2));
