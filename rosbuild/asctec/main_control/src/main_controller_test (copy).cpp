...
void compute_force(Eigen::Vector3d & force, double &yaw_torque)
{
	...
	// case pos_on == 3: control based on velocity error, but absolute z height also controlled
	// used for leader in teleoperation case, otherwise leader falls down
	else if(controller.pos_on == 3) {
		force = - coefs.Kv*controller.ev;
		force(2) = -coefs.Kp*controller.ep(2) - coefs.Kv*controller.ev(2) - coefs.KIz*controller.eIz*z(2);
	}
